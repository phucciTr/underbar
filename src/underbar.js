(function() {
  ...
  _.each = function(collection, iterator) {
    ...
    if (Array.isArray(collection)) {
      for (let i = 0; i < collection.length; i++) {
        ...
        iterator(current, i, collection);
      }

    } else {
      for (let key in collection) {
        let currentVal = collection[key];
        iterator(currentVal, key, collection);
      }
    }
  };
  ...
  _.filter = function(collection, test) {
    ...
    _.each(collection, function(current) {
      if (test(current)) { truths.push(current); }
    });
  ...
  _.reject = function(collection, test) {
    ...
    return _.filter(collection, function(current) {
  ...
  // to reduce. If (no starting value) is passed, the (1st element) is used as
  // the (accumulator), and is (never passed) to the (iterator). In other words, in
  // the case where a (starting value is not passed), the (iterator is not invoked)
  // until the (second element), with the (1st element) as its (2nd argument).
  //
  // Example:
  //   var numbers = [1,2,3];
  ...
  _.every = function(collection, iterator) {
    ...
    if (!iterator) {
      return _.reduce(collection, function(isTrue, current) {
      ...
      let areAllTruthy = true;

      _.each(collection, function(current) {
        if (!iterator(current)) { areAllTruthy = false; }
      });
  ...
  _.extend = function(obj) {
    ...
    _.each(arguments, function(currentObj, index) {
      if (index > 0) { Object.assign(obj, currentObj); }
    });
    ...
    _.each(arguments, function(currentObj, index) {
      ...
      if (index > 0) {
        for (let key in currentObj) {
          if (!(key in obj)) { obj[key] = currentObj[key]; }
        }
      }
    });
